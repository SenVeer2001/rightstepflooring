...
interface LineItem {
  ...
  taxable: boolean
  color?: string
  isDeleted?: boolean  // 👈 ADDED: Track deleted items
  isModified?: boolean  // 👈 ADDED: Track modified items
  originalData?: Partial<LineItem>  // 👈 ADDED: Store original values for change order
}
...
interface Estimate {
  ...
  imageUrl?: string
  items: LineItem[]
  hasChangeOrder?: boolean  // 👈 ADDED: Flag for change orders
  changeOrderHistory?: ChangeOrderRecord[]  // 👈 ADDED: History
}
...
export function EstimatesView() {
  ...
  const [estimates, setEstimates] = useState<Estimate[]>([
    {
      ...
      hasChangeOrder: false,  // 👈 ADDED
      changeOrderHistory: [],  // 👈 ADDED
  ...
  const [isAddEstimateMenuOpen, setIsAddEstimateMenuOpen] = useState(false)
  ...
  const handleCreateNewEstimate = () => {
    ...
    setIsAddEstimateMenuOpen(false)
  }
  ...
  const handleCreateEstimateCopy = () => {
    if (!selectedEstimate) return

    const copiedEstimate: Partial<Estimate> = {
      leadId: selectedEstimate.leadId,
      leadName: selectedEstimate.leadName,
      title: `${selectedEstimate.title} (Copy)`,
      description: selectedEstimate.description,
      clientEmail: selectedEstimate.clientEmail,
      clientPhone: selectedEstimate.clientPhone,
      serviceAddress: selectedEstimate.serviceAddress,
      status: 'pending',
      hasChangeOrder: false,
      changeOrderHistory: [],
  ...
  // 👇 UPDATED: Mark as deleted instead of removing
  const handleRemoveItem = (itemId: string) => {
    ...
    if (activeTab === 'view' && selectedEstimate) {
      ...
      const changeRecord: ChangeOrderRecord = {
        ...
        type: 'delete',
        itemId: itemId,
        oldValue: selectedEstimate.items.find(i => i.id === itemId),
      }
  ...
  const handleAddItem = () => {
    ...
    if (activeTab === "view" && selectedEstimate) {
      setSelectedEstimate({
        ...selectedEstimate,
        items: [...selectedEstimate.items, createdLineItem],
      })
    } else {
      setNewEstimate({
        ...newEstimate,
        items: [...(newEstimate.items || []), createdLineItem],
      })
    }
  ...
  const handleSaveItem = () => {
    ...
    if (activeTab === "view" && selectedEstimate) {
      // Add to change order history if modified
      const changeRecord: ChangeOrderRecord = {
        id: Date.now().toString(),
        date: new Date().toISOString(),
        type: 'modify',
        itemId: editingItemData.id,
        oldValue: selectedEstimate.items.find(i => i.id === editingItemData.id),
  ...
  return (
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-4">
            {activeTab === 'view' && selectedEstimate && (
              <button onClick={() => { navigate("/estimates") }} className="p-2 hover:bg-gray-100 rounded-lg transition">
    ...
    `}
          ...
          <div className="relative">
            <button
              onClick={() => setIsAddEstimateMenuOpen(prev => !prev)}
              className={`px-4 py-2 font-semibold border-b-2 flex items-center gap-2
      ${activeTab === "add"
                  ? "text-primary border-primary"
                  : "text-gray-600 border-transparent hover:text-gray-900"
                }`}
            ...
            {isAddEstimateMenuOpen && (
              <div className="absolute top-full mt-2 w-48 bg-white border rounded-lg shadow-lg z-50">
                <button
                  onClick={handleCreateNewEstimate}
                  className="w-full text-left flex gap-1 items-center px-4 py-2 text-sm rounded-lg hover:bg-gray-100 hover:text-primary "
                >
                  <PlusCircle size={16} /> New estimate
                </button>

                <button
                  onClick={handleCreateEstimateCopy}
                  className="w-full flex gap-1 items-center text-left px-4 py-2 text-sm rounded-lg hover:bg-gray-100 hover:text-primary"
                >
                  <CopyIcon size={16} /> Make a copy
                </button>
              </div>
            )}
          </div>
        ...
        <div className="">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            ...
            <button
              ...
              className="px-4 py-2 rounded-full border border-primary text-sm font-semibold hover:bg-primary hover:border-primary hover:text-white"
            >
              Price book
            </button>
